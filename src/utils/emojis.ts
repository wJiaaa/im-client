 ...
 * 表情对象，存储各个表情对应的图片链接
 */
const imgEmoji: { [key: string]: string } = {
  ...
  '[考虑中]': "<img style='width:30px' src='/src/assets/gif/90.gif'>",
  '[魔鬼笑]': "<img style='width:30px' src='/src/assets/gif/91.gif'>"
};

const emojisKeys = Object.keys(imgEmoji);
 ...
 */
export function textReplaceEmoji(content: string) {
  return content.replace(new RegExp(`(${regEmoji})`, 'g'), ($0, $1) => {
    let content = imgEmoji[$1];
    content = content.replace('<img', '<img class="emoji"');
    return content;
  });
}
